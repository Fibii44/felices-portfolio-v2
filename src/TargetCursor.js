...
const TargetCursor = ({
    ...
    const enterHandler = e => {
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          const { cornerSize: resetCornerSize } = constants;
          const positions = [
            { x: -resetCornerSize * 1.5, y: -resetCornerSize * 1.5 },
            { x: resetCornerSize * 0.5, y: -resetCornerSize * 1.5 },
            { x: resetCornerSize * 0.5, y: resetCornerSize * 0.5 },
            { x: -resetCornerSize * 1.5, y: resetCornerSize * 0.5 }
          ];
          const tl = gsap.timeline();
          cornerNodes.forEach((corner, index) => {
            tl.to(corner, { x: positions[index].x, y: positions[index].y, duration: 0.45, ease: 'power2.out' }, 0);
          });
        }
  ...
  return (
    <div
      ref={cursorRef}
      className="fixed top-0 left-0 w-0 h-0 pointer-events-none z-[11000]"
